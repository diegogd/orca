...
describe('buildRuntimeWorktreePsSummaries', () => {
  ...
  it('carries tags for tagged workspaces and omits the key otherwise', () => {
    ...
    const tagged = resolved('repo-1::/workspace/tagged')
    ...
    const store = {
      getRepos: () => [],
      getWorktreeMeta: (id: string) => (id === tagged.id ? { tags: ['billing'] } : {}),
      getAllWorktreeMeta: () => ({}),
      getFolderWorkspaces: () => [],
      getProjectGroups: () => []
    } as unknown as RuntimeStore
    ...
    expect(summaries.get(tagged.id)?.tags).toEqual(['billing'])
